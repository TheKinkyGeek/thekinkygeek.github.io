...
Nach dem starten von `topping` im Setup Modus:

``` bash
$ python3 main.py --config "config/config.ini" setup
```

werden zunächst alle Module die angeschlossen und eingeschalten sind erkannt. Dazu gibt es die Info über die installierte Firmware.
